...
def link_pred(nfeat, train_data, val_data, test_data, nruns):
    ...
    def train_link_predictor(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % 10 == 0 and verbose:
                print(f"Epoch: {epoch:03d}, Train Loss: {loss:.3f}, val_acc: {val_acc:.4f}, val_auc:{val_auc:.4f}, val_ap:{val_ap:.4f} ")
    ...
    logging.info(f"Test mea ACC: {mean[0]*100:.1f} ± {std[0]*100:.1f} | AUC: {mean[1]:.4f} ± {std[1]:.4f} | AP: {mean[2]:.4f} ± {std[2]:.4f}")
    print(f"Test mea ACC: {mean[0]*100:.1f} ± {std[0]*100:.1f} | AUC: {mean[1]:.4f} ± {std[1]:.4f} | AP: {mean[2]:.4f} ± {std[2]:.4f}")
...
parser.add_argument('--carch', type=int, default=1)
...
parser.add_argument('--expID', type=float, default=1)
...
if args.task == "AD_gen":
    ...
    'carch':args.carch,               
    'ACC_mean': round(mean[0], 1),
    'ACC_std': round(std[0], 1),
    'AUC_mean': round(mean[1], 1),
    'AUC_std': round(std[1], 1),
    'AP_mean': round(mean[2], 1),
    'AP_std': round(std[2], 1),
    'seed': args.seed,
    'marks':args.marks       
    }
    csv_file = f"result/own/all_lp.csv"
