...
def link_pred(nfeat, train_data, val_data, test_data, nruns):
    ...
    def train_link_predictor(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            # logging.info(f'neg_edge_index:{neg_edge_index.shape},pos_edge_index:{train_data.edge_label_index}')
            ...
            val_acc,val_auc,val_ap= eval_link_predictor(model, val_data)

            if epoch % 10 == 0 and verbose:
                logging.info(f"Epoch: {epoch:03d}, Train Loss: {loss:.3f}, val_acc: {val_acc:.4f}, val_auc:{val_auc:.4f}, val_ap:{val_ap:.4f} ")
    ...
    def eval_link_predictor(model, data):
        ...
        pred = out.cpu().numpy()
        # print("pred",pred)
        # print("data.edge_label.cpu().numpy()",data.edge_label.cpu().numpy())
        
        # 

        pred = (pred > 0.5).astype(np.int32)
        acc = accuracy_score(data.edge_label.cpu().numpy(), pred)
        auc = roc_auc_score(data.edge_label.cpu().numpy(),pred)
        ap = average_precision_score(data.edge_label.cpu().numpy(),pred)
        return acc,auc,ap
    ...
    for _ in range(nruns):
        ...
        test_acc, test_auc,test_ap= eval_link_predictor(model, test_data)
        res.append((test_acc, test_auc, test_ap))
    res = np.array(res)
    mean, std = res.mean(0), res.std(0)
    logging.info(f"Test mea ACC: {mean[0]:.4f} ± {std[0]:.4f} | AUC: {mean[1]:.4f} ± {std[1]:.4f} | AP: {mean[2]:.4f} ± {std[2]:.4f}")
    return  res.mean(0)*100, res.std(0)*100
...
def pca_reduce_features(data, target_dim):
    def to_tensor(x):
...
def load_data(args,dataset,target_dim):
    data_graphsaint = ['flickr', 'reddit','finance', 'ogbn-arxiv']
    if dataset in data_graphsaint:
        # data = DataGraphSAINT(args.dataset)
        data = DataGraph(dataset,data_dir=args.data_dir)
        data_full = data.data_full
    else:
        data_full = get_dataset(dataset,dataset_dir=args.data_dir)
        data = Transd2Ind(data_full, keep_ratio=args.keep_ratio)
    data=pca_reduce_features(data,target_dim)
...
parser.add_argument("--method", type=str, default="SGDD", help="Method") #Full
parser.add_argument("--carch", type=str, default="15")
parser.add_argument("--task", type=str, default="CLP", help="Task for evaluation")
...
parser.add_argument('--log_dir', type=str, default='logs/cross', help='log directory')
...
path = os.path.join(args.log_dir,args.dataset)
...
logging.basicConfig(
    filename=f"{args.log_dir}/{args.dataset}/{args.method}/{args.dataset}-trans-{args.transductive}-{args.reduction_rate}-{args.task}-{args.seed}-{args.marks}.log",
    level=logging.INFO,
...
if args.task == "AD_gen":
    ...
    for data_name in args.target_datasets:
        logging.info(f'\n===> Transferring to target dataset: {data_name}')
        test_data =load_data(args,data_name,feat_syn.shape[1])

        mean,std=link_pred(feat_syn.shape[1], train_data, val_syn, test_data, args.nruns)
        result_row = {
        'source': args.dataset, 
        'rate': args.reduction_rate,                
        'target': data_name,                        
        'carch':args.carch,               
        'ACC_mean': f'{round(mean[0], 1)}±{round(std[0], 1)}',
        'AUC_mean': f'{round(mean[1], 1)}±{round(std[1], 1)}',
        'AP_mean': f'{round(mean[2], 1)}±{round(std[2], 1)}',
        'seed': args.seed,
        'method': args.method, 
        'marks':args.marks       
        }
        csv_file = f"result/cross/{args.method}_cross_lp.csv"
