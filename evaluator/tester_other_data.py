...
from tester_cross_data import autoencode_features, train_encoder_classifier, test_classifier,test_classifier_v2,test_classifier_v3
...
class Evaluator:
    ...
    def __init__(self, data, args, device="cuda", **kwargs):
        ...
        self.args = args
        self.device = device
        if args.method in ["SGDD","DosCond"] and args.dataset in ["ogbn-arxiv"]:
            self.labels_syn = torch.LongTensor(self.generate_labels_syn(data)).to(device)
            n=self.labels_syn.shape[0]
            print("self.labels_syn.shape[0]",self.labels_syn.shape[0])
        else:
            self.labels_syn = torch.LongTensor(self.generate_labels_syn2(data)).to(device)
            print("self.labels_syn.shape[0]",self.labels_syn.shape[0])
        # n1 = int(data.feat_train.shape[0] * args.reduction_rate)
        # n=self.labels_syn.shape[0]
        n = int(len(data.idx_train) * args.reduction_rate)
        # n=n2 if args.method in ["SGDD","DosCond"] else n1
        # n2=int(len(data.idx_train) * args.reduction_rate)
        # n = int(len(data.idx_train) * args.reduction_rate)
        # n = min(int(len(data.idx_train) * args.reduction_rate), self.labels_syn.shape[0])
        # print("n",n2)
        ...
        logging.info(f"adj_param:{self.adj_param.shape}, feat_syn:{self.feat_syn.shape}")

    def reset_parameters(self):
        self.adj_param.data.copy_(torch.randn(self.adj_param.size()))
        self.feat_syn.data.copy_(torch.randn(self.feat_syn.size()))

    def generate_labels_syn2(self, data):
    ...
    def load_data(self,dataset):
        ...
        if dataset in data_graphsaint:
            ...
            data = DataGraph(dataset,data_dir=self.args.data_dir)
            data_full = data.data_full
        else:
            data_full = get_dataset(dataset,dataset_dir=self.args.data_dir)
            data = Transd2Ind(data_full, keep_ratio=self.args.keep_ratio)
        return data
    ...
    def get_syn_data(self, model_type=None, coreset=False):
        ...
        if coreset:
            ...
            idx = np.load(
                f"{args.save_dir}/{args.method}/idx_{args.dataset}_{args.reduction_rate}_{args.method}_{args.carch}.npy"
            )
            ...
            labels_syn = self.labels_syn#torch.LongTensor(self.generate_labels_syn(args, data)).cuda()
        else:
            ...
            #         map_location="cuda",
            #     )
            file_str = '_best_ntk_score_' if args.method == 'SFGC' else '_best_eval_acc_'
            if args.method in ['SFGC']: 
                save_dir = args.save_dir
                method = args.method
                dataset = args.dataset
                reduction_rate = args.reduction_rate
                file_dir=f'{dataset}-reduce_{reduction_rate}-distillation'
                # adj_syn = torch.load(adj_file, map_location='cuda')
                feat_file = os.path.join(save_dir,method,file_dir, f'feat_{dataset}_{reduction_rate}{file_str}{args.carch}.pt')
                feat_syn = torch.load(feat_file, map_location='cuda')
                adj_syn = torch.eye(feat_syn.shape[0])
                label_file = os.path.join(save_dir, method, file_dir,f'label_{dataset}_{reduction_rate}{file_str}{args.carch}.pt')
                ...
                file_dir=f'{dataset}-reduce_{reduction_rate}-distillation'
                # adj_syn = torch.load(adj_file, map_location='cuda')
                feat_file = os.path.join(save_dir,method,file_dir, f'feat_{dataset}_{reduction_rate}{file_str}{args.carch}_ours.pt')
                feat_syn = torch.load(feat_file, map_location='cuda')
                adj_syn = torch.eye(feat_syn.shape[0])
                label_file = os.path.join(save_dir, method, file_dir,f'label_{dataset}_{reduction_rate}{file_str}{args.carch}_ours.pt')
                labels_syn = torch.load(label_file, map_location='cuda')
                if dataset not in ['finance']:
                    labels_syn = labels_syn.argmax(dim=1)
                print(f"adj_syn:{adj_syn.shape}, feat_syn:{feat_syn.shape},labels_syn:{labels_syn.shape}") 
            elif args.method=="CGC":
                graph_file = f'{args.save_dir}/{args.method}/{args.dataset}/{args.dataset}_{args.reduction_rate}.pt' if args.generate_adj == 1 else f'{args.save_dir}/{args.method}/{args.dataset}/{args.dataset}_noadj_{args.reduction_rate}.pt'
                data = torch.load(graph_file,map_location='cuda')
                feat_syn = data.x 
                labels_syn = data.y
                if args.generate_adj == 1:
                    from torch_geometric.utils import to_dense_adj
                    adj_syn= to_dense_adj(data.edge_index,max_num_nodes=feat_syn.size(0))[0] 
                else:
                    adj_syn = torch.eye(feat_syn.shape[0]) 
                print(f"adj_syn:{adj_syn.shape}, feat_syn:{feat_syn.shape},labels_syn:{labels_syn.shape}") 
    ...
    def pca_reduce_features(self, data, target_dim):
        def to_tensor(x):
            if isinstance(x, np.ndarray):
                x = torch.from_numpy(x)
            return x.to('cpu').float()

        feat_train = to_tensor(data.feat_train)
        feat_test = to_tensor(data.feat_test)
        feat_full = to_tensor(data.feat_full)

        all_feats = torch.cat([feat_train, feat_test, feat_full], dim=0)
        pca = PCA(n_components=target_dim)
        pca.fit(all_feats.numpy())

        def reduce_split(x):
            x_cpu = to_tensor(x)  # 
            x_pca = pca.transform(x_cpu.numpy())  # PCA -> ndarray
            return x_pca #torch.tensor(x_pca).float().to(self.device)  

        data.feat_train = reduce_split(data.feat_train)
        data.feat_test = reduce_split(data.feat_test)
        data.feat_val = reduce_split(data.feat_val)
        data.feat_full = reduce_split(data.feat_full)
        return data
    ...
    def run_cross_dataset_transfer(args, datasets, source_dataset_name,encode,load_data_fn):
        results = []
        device = args.device
        hidden_dim = args.hidden_dim
        embed_dim = args.embed_dim
        # encoder_save_path = f'encoder_{source_dataset_name}.pth'

        source_data = load_data_fn(args,source_dataset_name)
        target_dim = source_data.feat_train.shape[1]
    
        # 2. Evaluate on each target dataset
        for target_name in datasets:
            if target_name == source_dataset_name:
                continue
            print(f'\n===> Transferring to target dataset: {target_name}')
            acc_list = []
            for run in range(args.nruns):
                target_data = load_data_fn(args,target_name)
                target_data = autoencode_features(target_data, target_dim=target_dim, device=device)
                acc = test_classifier(args,target_data, encoder=encode, device=device)
                acc_list.append(acc)
            acc_mean = sum(acc_list) / len(acc_list)
            acc_std = (sum((x - acc_mean) ** 2 for x in acc_list) / len(acc_list)) ** 0.5
            results.append({
        ...
        df.to_csv(f'result/{args.method}_{args.reduction_rate}_{args.seed}_{args.result_csv}', index=False)
        print(f'\nResults saved to {args.result_csv}')
    
    def train(self, verbose=True):
        args = self.args
        data = self.data
        target_dim=data.feat_train.shape[1]
        ...
        for model_type in models_name:
            res = []
            nlayer = 2
            # target_data = self.load_data(dataset=target_name)
            # res,model=self.test(nlayer, verbose=True, model_type=model_type)
        
            # 2. Evaluate on each target dataset
            for target_name in args.target_datasets:
                if target_name ==args.dataset:
                    continue
                logging.info(f'\n===> Transferring to target dataset: {target_name}')
                acc_list = []
                target_data = self.load_data(dataset=target_name)
                target_data=self.pca_reduce_features(target_data, target_dim=target_dim)

                for run in range(3): 
                    #args.nruns
                    res_acc,model=self.test(nlayer, verbose=True, model_type=model_type)
                    res.append(res_acc)
                    logging.info(model)
                    # target_data = self.load_data(dataset=target_name)
                    # target_data=self.pca_reduce_features(target_data, target_dim=target_dim)
                    acc = test_classifier(args,target_name,target_data, encoder=model, device=self.device)
                    # acc = test_classifier(args,target_name,target_data, encoder=model, device=self.device)
                    acc_list.append(acc)
                acc_mean = sum(acc_list) / len(acc_list)
                acc_std = (sum((x - acc_mean) ** 2 for x in acc_list) / len(acc_list)) ** 0.5
                result_row ={
                    "source": args.dataset,
                    "reduce_rate":args.reduction_rate,
                    "carch":args.carch,
                    "target": target_name,
                    "acc_mean": f"{acc_mean*100:.1f}",
                    "acc_std": f"{acc_std*100:.1f}",
                    "seed":args.seed,
                    "acc_list": acc_list,
                    "marks":args.marks
                }
                # 3. Save results
                result_path = f'result/cross/nc/{args.method}_{args.result_csv}'
                file_exists = os.path.exists(result_path)
                df = pd.DataFrame([result_row])
                df.to_csv(result_path, mode='a', header=not file_exists, index=False)

            res = np.array(res)
            logging.info(f"Test/Train Mean Accuracy: {repr([res.mean(0), res.std(0)])}")
        ...
        logging.info("Final result:", final_res)
