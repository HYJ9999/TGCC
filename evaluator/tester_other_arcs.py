...
class Evaluator:
    ...
    def train(self, verbose=True):
        ...
        for model_type in models_name:
            ...
            results.append(f"{res.mean(0)[0]*100:.1f} ± {res.std(0)[0]*100:.1f}")
        ...
        logging.info(f"Final result: {final_res}")
        logging.info("\t".join(models_name))
        results="\t".join(results)
        logging.info(f"test acc mean: {results}")
