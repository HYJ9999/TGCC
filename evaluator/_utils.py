...
def get_syn_data(args, carch=1): # None
    ...
    if args.method in ['SFGC', 'GEOM']:
        ...
        graph_file = f'{args.save_dir}/{args.method}/{args.dataset}/{args.dataset}_{args.reduction_rate}.pt' 
        ...
        if carch is None:
            carch = get_files_and_seed(args, args.method, args.dataset, args.reduction_rate, file_str)
        else:
            carch = args.carch
        if args.method in ['kcenter','random','herding']:
            ...
            idx = np.load(
                f"{args.save_dir}/{args.method}/idx_{args.dataset}_{args.reduction_rate}_{args.method}_{carch}.npy"
            )
            ...
            adj_syn = torch.load(f'{args.save_dir}/{args.method}/adj_{args.dataset}_{args.reduction_rate}_{carch}.pt', map_location='cuda')
            feat_syn = torch.load(f'{args.save_dir}/{args.method}/feat_{args.dataset}_{args.reduction_rate}_{carch}.pt', map_location='cuda')
