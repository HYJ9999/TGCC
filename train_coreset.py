...
import os
import logging
from utils_csv import append_result
...
logging.basicConfig(
    filename=f"{args.log_dir}/{args.dataset}/{args.dataset}-{args.method}-{args.reduction_rate}-{args.seed}.log",
    level=logging.INFO,
...
append_result(args,res)

res = np.array(res)
print('Mean accuracy:', repr([res.mean(), res.std()]))
logging.info(f"Mean accuracy:{repr([res.mean(), res.std()])}")
