...
class GCN(nn.Module):
    ...
    def test(self, idx_test):
        ...
        print("Test set results:",
                ...
                "Mean accuracy=",repr([res.mean(), res.std()]))
    ...
    @torch.no_grad()
    def get_emdedding(self, x, adj, get_embedding=False):
        self.eval()
        for ix, layer in enumerate(self.layers):
            x = layer(x, adj)
            if ix != len(self.layers) - 1:
    ...
    def fit(self,features, adj, labels, idx_train, idx_val, train_iters=200, normalize=True, verbose=False,adj_val=False, **kwargs):
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
