...
class GCN(nn.Module):
    ...
    def forward_casual(self,x1,adj1,x2,adj2):
        h1=self.forward( x1, adj1,get_embedding=True)
        h2=self.forward( x2, adj2,get_embedding=True)
    ...
    def forward_embedding(self, x, adj):
        for ix, layer in enumerate(self.layers):
            x = layer(x, adj)
            if ix != len(self.layers) - 1:
                x = self.bns[ix](x) if self.with_bn else x
    ...
    def _train_without_val2(self, labels, idx_train, train_iters, verbose):
        ...
        for i in range(train_iters):
            ...
            if verbose and i % 10 == 0:
                ...
                logging.info('Epoch {}, training loss: {}'.format(i, loss_train.item()))

        self.eval()
        output = self.forward(self.features, self.adj_norm)
        self.output = output

    def fit_with_val(self, features, adj, labels, data, train_iters=200, initialize=True, verbose=True, normalize=True, patience=None, noval=False, **kwargs):
    ...
    def _train_with_val(self, labels, data, train_iters, verbose, adj_val=False):
        ...
        if verbose:
            ...
            logging.info('=== picking the best model according to the performance on validation ===')
    ...
    @torch.no_grad()
    def get_embedding(self, x, adj,get_embedding=False):
        for ix, layer in enumerate(self.layers):
            x = layer(x, adj)
            if ix != len(self.layers) - 1:
                x = self.bns[ix](x) if self.with_bn else x
    ...
    @torch.no_grad()
    def predict_em(self, features=None, adj=None):
        ...
        if features is None and adj is None:
            ...
            return self.forward(self.features, self.adj_norm,get_embedding=True)  
