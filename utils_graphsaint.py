...
class DataGraphSAINT:
    ...
    def retrieve_class_sampler(self, c, adj, transductive, num=256, args=None):
        if args.nlayers == 1:
            sizes = [30]
        if args.nlayers == 2:
            if args.dataset in ['reddit', 'flickr','finance']:
                if args.option == 0:
                    sizes = [15, 8]
                if args.option == 1:
                    sizes = [20, 10]
        ...
        if self.samplers is None:
            ...
            for i in range(self.nclass):
                ...
                if len(node_idx) == 0:
                    continue
                print(f"[Sampler Init] class {i}: {len(node_idx)} samples")
                ...
                self.samplers.append(NeighborSampler(adj,
                                    ...
                                    shuffle=True))
                self.class_to_sampler_index[i] = sampler_idx 
                sampler_idx += 1 
        batch = np.random.permutation(self.class_dict2[c])[:num]

        if c not in self.class_to_sampler_index:
            raise ValueError(f"Class {c} has no sampler initialized (no training samples?)") 

        sampler_idx = self.class_to_sampler_index[c] 
        out = self.samplers[sampler_idx].sample(batch) 
