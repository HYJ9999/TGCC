...
from util_tools.hetero_data_loader import hetero_dl
...
def load_hetero_data(dataset, data_dir, truncate=0):
    dl = hetero_dl(data_dir + "/" + dataset)
    index_dir = os.path.join(data_dir, dataset)
    train_index_path = os.path.join(index_dir, "train_idx.npy")
    val_index_path = os.path.join(index_dir, "val_idx.npy")
    test_index_path = os.path.join(index_dir, "test_idx.npy")
    ...
    for i in range(len(dl.nodes["count"])):
        th = dl.nodes["attr"][i]
        if truncate:
            # max_dim = dl.nodes["count"][0]
            if th is None and i == 0:
                features.append(sp.eye(dl.nodes["count"][i]).toarray())
                max_dim = dl.nodes["count"][i]
            elif th is not None:
                features.append(th)
            else:
                features.append(np.zeros((dl.nodes["count"][i], max_dim)))
        else:
            if th is None:
                features.append(sp.eye(dl.nodes["count"][i]).toarray())
                max_dim = (
                    dl.nodes["count"][i] if dl.nodes["count"][i] > max_dim else max_dim
                )
            else:
                features.append(th)
    ...
    for i, feat in enumerate(features):
        if feat.shape[1] < max_dim:
            features[i] = np.hstack(
                (feat, np.zeros((feat.shape[0], max_dim - feat.shape[1])))
            )
    features = np.concatenate(features, axis=0)
    adjM = sum(dl.links["data"].values())
    labels = np.zeros((dl.nodes["count"][0], dl.labels_train["num_classes"]), dtype=int)
    if (
        os.path.exists(train_index_path)
        and os.path.exists(val_index_path)
        and os.path.exists(test_index_path)
    ):
        # Load existing indexes
        train_idx = np.load(train_index_path)
        val_idx = np.load(val_index_path)
        test_idx = np.load(test_index_path)
    else:
        val_ratio = 0.2
        ...
        np.save(train_index_path, train_idx)
        np.save(val_index_path, val_idx)
...
def load_credit(
    ...
    idx_features_labels = pd.read_csv(
        os.path.join(data_dir, dataset, "{}.csv".format(dataset))
    )
    ...
    # build relationship
    if os.path.exists(f"{data_dir}/{dataset}/{dataset}_edges.txt"):
        edges_unordered = np.genfromtxt(
            f"{data_dir}/{dataset}/{dataset}_edges.txt"
        ).astype("int")
    else:
        edges_unordered = build_relationship(idx_features_labels[header], thresh=0.7)
        np.savetxt(f"{data_dir}/{dataset}/{dataset}_edges.txt", edges_unordered)
    ...
    return adj, features, labels, train_idx, val_idx, test_idx, sens
...
class DataGraph:
    ...
    def __init__(self, dataset, **kwargs):
        ...
        if dataset in ["ACM", "DBLP", "Freebase", "IMDB"]:
            ...
            adj_full, feat, labels, idx_train, idx_val, idx_test = load_hetero_data(
                data_dir=data_dir, dataset=dataset
            )
            self.nnodes = adj_full.shape[0]
            self.nclass = len(np.unique(labels))
        elif dataset in ["credit"]:
            adj_full, feat, labels, idx_train, idx_val, idx_test, sens = load_credit(
                dataset, data_dir
            )
            self.sens = sens
            self.nnodes = adj_full.shape[0]
    ...
    def get_cluster_centers(node_indexes, features, cluster_num):
        ...
        kmeans = KMeans(n_clusters=cluster_num, random_state=0).fit(node_features)
    ...
    def retrieve_class_sampler(
        self, c, adj, transductive, num=256, features=None, args=None
    ):
        if args.nlayers == 1:
            sizes = [30]
        if args.nlayers == 2:
